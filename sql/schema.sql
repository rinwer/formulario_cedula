...
-- Los roles "coordinador" y "visualizador" se agregaron despues de la
-- corrida inicial de este script (cuando el check inline solo admitia
-- administrador/lider_cuadrilla): se reemplaza esa restriccion por una
-- que tambien los permita, en vez de dejar el check viejo bloqueando
-- altas con esos roles nuevos.
-- - coordinador: Trabajos/Programacion/Daily, pero NO gestionar
--   usuarios (eso queda solo para administrador).
-- - visualizador: solo lectura, unicamente puede ver el Daily.
alter table public.profiles drop constraint if exists profiles_role_check;
alter table public.profiles add constraint profiles_role_check
  check (role in ('administrador', 'coordinador', 'visualizador', 'lider_cuadrilla'));

comment on table public.profiles is 'Perfil y rol de cada usuario autenticado. 1:1 con auth.users.';
comment on column public.profiles.role is 'Rol del usuario: administrador (gestiona todo, incl. usuarios), coordinador (Trabajos/Programacion/Daily, no usuarios), visualizador (solo lectura del Daily) o lider_cuadrilla.';
